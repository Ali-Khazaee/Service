...
module.exports = (Client) =>
{
    ...
    Client.On(Packet.SocialPostList, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('account').find({ _id: Message.ID }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post').find({ $and: [ { Owner: Message.ID }, { Delete: { $exists: false } } ] }).toArray((Error2, Result2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostList, Error: Error2 })
                    return Client.Send(Packet.SocialPostList, ID, { Result: -1 })
                }

                Client.Send(Packet.SocialPostList, ID, { Result: 0, Message: Result2 })
    ...
    Client.On(Packet.SocialPostLike, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_like').find({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error2, Result2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostCommentLike, Error: Error2 })
                ...
                DB.collection('post_like').insertOne({ ID: Message.ID, Owner: MongoID(Client.__Owner), Time: Misc.TimeMili() }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostLike, Error: Error3 })
                        return Client.Send(Packet.SocialPostLike, ID, { Result: -1 })
                    }
    ...
    Client.On(Packet.SocialPostDisLike, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_like').find({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error2, Result2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostDisLike, Error: Error2 })
                    return Client.Send(Packet.SocialPostDisLike, ID, { Result: -1 })
                }
                ...
                DB.collection('post_like').deleteOne({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) } ] }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostDisLike, Error: Error3 })
                        return Client.Send(Packet.SocialPostDisLike, ID, { Result: -1 })
                    }
    ...
    Client.On(Packet.SocialPostDelete, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Owner: MongoID(Client.__Owner) }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post').updateOne({ ID: Message.ID }, { $set: { Delete: Misc.TimeMili() } }, (Error2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error2 })
                    return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                }

                DB.collection('post_like').updateMany({ ID: Message.ID }, { $set: { Delete: Misc.TimeMili() } }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error3 })
                        return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                    }

                    DB.collection('post_report').updateMany({ ID: Message.ID }, { $set: { Delete: Misc.TimeMili() } }, (Error4) =>
                    {
                        if (Misc.IsDefined(Error4))
                        {
                            Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error4 })
                            return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                        }

                        DB.collection('post_view').updateMany({ ID: Message.ID }, { $set: { Delete: Misc.TimeMili() } }, (Error5) =>
                        {
                            if (Misc.IsDefined(Error5))
                            {
                                Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error5 })
                                return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                            }

                            DB.collection('post_comment').updateMany({ ID: Message.ID }, { $set: { Delete: Misc.TimeMili() } }, (Error6) =>
                            {
                                if (Misc.IsDefined(Error6))
                                {
                                    Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error6 })
                                    return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                                }

                                DB.collection('post_comment_reply').updateMany({ ID: Message.ID }, { $set: { Delete: Misc.TimeMili() } }, (Error7) =>
                                {
                                    if (Misc.IsDefined(Error7))
                                    {
                                        Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error7 })
                                        return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                                    }

                                    DB.collection('post_comment_like').updateMany({ ID: Message.ID }, { $set: { Delete: Misc.TimeMili() } }, (Error8) =>
                                    {
                                        if (Misc.IsDefined(Error8))
                                        {
                                            Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error8 })
                                            return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                                        }

                                        DB.collection('post_bookmark').deleteMany({ ID: Message.ID }, (Error9) =>
                                        {
                                            if (Misc.IsDefined(Error9))
                                            {
                                                Misc.Analyze('DBError', { Tag: Packet.SocialPostDelete, Error: Error9 })
                                                return Client.Send(Packet.SocialPostDelete, ID, { Result: -1 })
                                            }
    ...
    Client.On(Packet.SocialPostReport, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_report').insertOne({ ID: Message.ID, Owner: MongoID(Client.__Owner), Reason: Message.Reason, Time: Misc.TimeMili() }, (Error2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostReport, Error: Error2 })
                    return Client.Send(Packet.SocialPostReport, ID, { Result: -1 })
                }
    ...
    Client.On(Packet.SocialPostView, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_view').find({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error2, Result2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostView, Error: Error2 })
                    return Client.Send(Packet.SocialPostView, ID, { Result: -1 })
                }
                ...
                DB.collection('post_view').insertOne({ ID: Message.ID, Owner: MongoID(Client.__Owner), Time: Misc.TimeMili() }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostView, Error: Error3 })
                        return Client.Send(Packet.SocialPostView, ID, { Result: -1 })
                    }
    ...
    Client.On(Packet.SocialPostComment, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_comment').insertOne({ ID: Message.ID, Message: Message.Message, Owner: MongoID(Client.__Owner), Time: Misc.TimeMili() }, (Error2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostComment, Error: Error2 })
                    return Client.Send(Packet.SocialPostComment, ID, { Result: -1 })
                }
    ...
    Client.On(Packet.SocialPostCommentDelete, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post_comment').find({ $and: [ { _id: Message.ID }, { Owner: MongoID(Client.__Owner) }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_comment').updateOne({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) } ] }, { $set: { Delete: Misc.TimeMili() } }, (Error2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostCommentDelete, Error: Error2 })
                    return Client.Send(Packet.SocialPostCommentDelete, ID, { Result: -1 })
                }
    ...
    Client.On(Packet.SocialPostCommentLike, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post_comment').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_comment_like').find({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error2, Result2) =>
            {
                ...
                DB.collection('post_comment_like').insertOne({ ID: Message.ID, Owner: MongoID(Client.__Owner), Time: Misc.TimeMili() }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostCommentLike, Error: Error3 })
                        return Client.Send(Packet.SocialPostCommentLike, ID, { Result: -1 })
                    }
    ...
    Client.On(Packet.SocialPostCommentDisLike, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post_comment').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_comment_like').find({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error2, Result2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostCommentDisLike, Error: Error2 })
                    return Client.Send(Packet.SocialPostCommentDisLike, ID, { Result: -1 })
                }
                ...
                DB.collection('post_comment_like').deleteOne({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) } ] }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostCommentDisLike, Error: Error3 })
                        return Client.Send(Packet.SocialPostCommentDisLike, ID, { Result: -1 })
                    }
    ...
    Client.On(Packet.SocialPostCommentReply, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post_comment').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            // FixMe check filtered words & is user blocked
            DB.collection('post_comment_reply').insertOne({ ID: Message.ID, Message: Message.Message, Owner: MongoID(Client.__Owner), Time: Misc.TimeMili() }, (Error2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostCommentReply, Error: Error2 })
                    return Client.Send(Packet.SocialPostCommentReply, ID, { Result: -1 })
                }
    ...
    Client.On(Packet.SocialPostBookmark, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_bookmark').find({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) } ] }).limit(1).project({ _id: 1 }).toArray((Error2, Result2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostBookmark, Error: Error2 })
                    return Client.Send(Packet.SocialPostBookmark, ID, { Result: -1 })
                }
                ...
                DB.collection('post_bookmark').insertOne({ ID: Message.ID, Owner: MongoID(Client.__Owner), Time: Misc.TimeMili() }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostBookmark, Error: Error3 })
                        return Client.Send(Packet.SocialPostBookmark, ID, { Result: -1 })
                    }
    ...
    Client.On(Packet.SocialPostUnBookmark, RateLimit(120, 60), (ID, Message) =>
    {
        ...
        DB.collection('post').find({ $and: [ { _id: Message.ID }, { Delete: { $exists: false } } ] }).limit(1).project({ _id: 1 }).toArray((Error, Result) =>
        {
            ...
            DB.collection('post_bookmark').find({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) } ] }).limit(1).project({ _id: 1 }).toArray((Error2, Result2) =>
            {
                if (Misc.IsDefined(Error2))
                {
                    Misc.Analyze('DBError', { Tag: Packet.SocialPostUnBookmark, Error: Error2 })
                    return Client.Send(Packet.SocialPostUnBookmark, ID, { Result: -1 })
                }
                ...
                DB.collection('post_bookmark').deleteOne({ $and: [ { ID: Message.ID }, { Owner: MongoID(Client.__Owner) } ] }, (Error3) =>
                {
                    if (Misc.IsDefined(Error3))
                    {
                        Misc.Analyze('DBError', { Tag: Packet.SocialPostUnBookmark, Error: Error3 })
                        return Client.Send(Packet.SocialPostUnBookmark, ID, { Result: -1 })
                    }
